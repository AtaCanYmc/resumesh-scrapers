...
async def main():
    ...
    try:
        ...
        logger.info(f"GitHub data successfully fetched: {len(github_data)} repositories found.")
    ...
    try:
        ...
        logger.info(f"Dev.to data successfully fetched: {len(devto_data)} articles found.")
    except ScraperError as e:
    ...
    try:
        ...
        logger.info(f"Medium data successfully fetched: {len(medium_data)} articles found.")
    except ScraperError as e:
    ...
    try:
        ...
        pypi_data = await pypi_scraper.fetch_data(github_username, package_names=pypi_packages)
        logger.info(f"PyPI data successfully fetched: {len(pypi_data)} packages processed.")
    except ScraperError as e:
